...
class TestingAsset(unittest.TestCase):
    ...
    def setUpClient(self):
        ...
        asset_object = binance_client.Asset(self.client,currency, precision=precision, purchase_amount=purchase_amount) 
    ...
    def test_historical_data(self):
        starting_cash = 1500
        data_set = self.get_test_set_hours("historical_data")
        dates, values, action_dates, total_profits, avaiable_cash, postion_worth = run_history.main(data_set,self.market_object,starting_cash)

        print(total_profits)
        self.assertTrue(total_profits == 5.0)
        self.assertTrue(len(action_dates) == 2)
        self.assertTrue(avaiable_cash == 1505)
